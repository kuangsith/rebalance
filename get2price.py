...
plt.plot(df['datetime'],df[f'Capital - {ticker1}'],label=ticker1)
plt.plot(df['datetime'],df[f'Capital - {ticker2}'],label=ticker2)
plt.plot(df['datetime'],df['Total Cap'],label = 'Rebalance port')
plt.title(f'Rebalancing: {ticker1} {int(100*ratio)}% , {ticker2} {int(100*(1-ratio))}%')
plt.legend()
plt.show()